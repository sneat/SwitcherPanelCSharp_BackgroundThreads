...
namespace SwitcherPanelCSharp
{
    public partial class SwitcherPanel : Form
    {
        ...
        public SwitcherPanel()
        {
            ...
            // This means we keep doing a lot of switching between main and background threads via async/await calls and this.Invoke()
            Initialise();
        }
        ...
        private async Task Initialise()
        {
            await Task.Run(() =>
            {
                ...
                m_switcherMonitor.SwitcherDisconnected += new SwitcherEventHandler((s, a) => this.Invoke((Action)(async () => await SwitcherDisconnected())));

                m_mixEffectBlockMonitor = new MixEffectBlockMonitor();
                m_mixEffectBlockMonitor.ProgramInputChanged += new SwitcherEventHandler((s, a) => this.Invoke((Action)(async () => await UpdateProgramButtonSelection())));
                m_mixEffectBlockMonitor.PreviewInputChanged += new SwitcherEventHandler(async (s, a) => await UpdatePreviewButtonSelection());
                m_mixEffectBlockMonitor.TransitionFramesRemainingChanged += new SwitcherEventHandler((s, a) => this.Invoke((Action)(async () => await UpdateTransitionFramesRemaining())));
                m_mixEffectBlockMonitor.TransitionPositionChanged += new SwitcherEventHandler((s, a) => this.Invoke((Action)(async () => await UpdateSliderPosition())));
                m_mixEffectBlockMonitor.InTransitionChanged += new SwitcherEventHandler((s, a) => this.Invoke((Action)(async () => await OnInTransitionChanged())));
            ...
            await SwitcherDisconnected();		// start with switcher disconnected
        }

        private void OnInputLongNameChanged(object sender, object args)
        {
            this.Invoke((Action)(async () => await UpdatePopupItems()));
        }

        private async Task SwitcherConnected()
        {
            if (SwitcherPanel.IsMainThread)
            {
                await Task.Run(SwitcherConnected);
                return;
            }
            ...
            this.Invoke((Action)(() => { MixEffectBlockSetEnable(true); }));
            await UpdatePopupItems();
            await UpdateTransitionFramesRemaining();
            await UpdateSliderPosition();
        }

        private async Task SwitcherDisconnected()
        {
            if (SwitcherPanel.IsMainThread)
            {
                await Task.Run(SwitcherDisconnected);
                return;
            }
        ...
        private async Task UpdatePopupItems()
        {
            if (SwitcherPanel.IsMainThread)
            {
                await Task.Run((Action)(async () => { await UpdatePopupItems(); }));
                return;
            }
            this.Invoke((Action)(() =>
            {
            ...
            await UpdateProgramButtonSelection();
            await UpdatePreviewButtonSelection();
        }

        private async Task UpdateProgramButtonSelection()
        {
            if (SwitcherPanel.IsMainThread)
            {
                await Task.Run((Action)(async () => { await UpdateProgramButtonSelection(); }));
                return;
            }
            long programId;
        ...
        private async Task UpdatePreviewButtonSelection()
        {
            if (SwitcherPanel.IsMainThread)
            {
                await Task.Run((Action)(async () => { await UpdatePreviewButtonSelection(); }));
                return;
            }
            long previewId;
        ...
        private async Task UpdateTransitionFramesRemaining()
        {
            if (SwitcherPanel.IsMainThread)
            {
                await Task.Run((Action)(async () => { await UpdateTransitionFramesRemaining(); }));
                return;
            }
        ...
        private async Task UpdateSliderPosition()
        {
            if (SwitcherPanel.IsMainThread)
            {
                await Task.Run((Action)(async () => { await UpdateSliderPosition(); }));
                return;
            }
        ...
        private async Task OnInTransitionChanged()
        {
            if (SwitcherPanel.IsMainThread)
            {
                await Task.Run((Action)(async () => { await OnInTransitionChanged(); }));
                return;
            }
            int inTransition;
        ...
        private async void comboBoxProgramSel_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private async void comboBoxPreviewSel_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private async void trackBarTransitionPos_Scroll(object sender, EventArgs e)
        {
            if (m_mixEffectBlock1 != null)
            {
